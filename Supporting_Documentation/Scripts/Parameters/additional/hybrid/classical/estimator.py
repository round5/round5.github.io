...
class BKZ:
    ...
    @staticmethod
    def R5ringSieve(beta, d, B=None, mode="classical"):
        c = {"classical": 0.2920,
             "quantum":   0.2650}
        c = c[mode]
    ...
    @staticmethod
    def R5nonringSieve(beta, d, B=None, mode="classical"):
        c = {"classical": 0.2920,
             "quantum":   0.2650}
        c = c[mode]

        return beta * ZZ(2)**RR(c*beta)

    @staticmethod
    def R5Enum(beta, d, B=None, mode="clasical"):
        cost = RR(0.18728*beta*log(beta, 2) - 1.0192*beta + 16.1)
        if mode == "classical":
            return ZZ(2)**cost
        if mode == "quantum":
            cost /= 2
